...
class InfluxDuplicator(QWidget):
    ...
    def write_duplicate(self, sensors: dict, actuators: dict):
        ...
        try:
            # measurement name can stay as-is; the important part is tagging location
            p = self.Point("greenhouse_room_2") \
            ...
            # mirror actuators too (matches room_1 schema in your screenshot)
            for k, v in (actuators or {}).items():
                try:
                    p = p.field(k, float(v))
                except Exception:
                    # skip non-numeric actuator values
                    pass
...
class PlcPanel(QGroupBox):
    applyOverride = Signal(str, float)   # sensor_key, value
    clearOverride = Signal(str)          # sensor_key
    ...
    def __init__(self, title, sensor_key, value_unit, gauge_max,
        ...
        self.override_in = QLineEdit()
        self.override_in.setPlaceholderText("Override value")
        self.apply_btn = QPushButton("Apply")
        self.reset_btn = QPushButton("Reset")

        o_row = QHBoxLayout()
        o_row.addWidget(self.override_in)
    ...
    def _apply_clicked(self):
        try:
            val = float(self.override_in.text())
        except ValueError:
            QMessageBox.warning(self, "Invalid override", "Enter a numeric value.")
            return
        self.applyOverride.emit(self.sensor_key, val)
...
class DashboardWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.co2_panel = PlcPanel("🟢 CO₂ PLC", "co2", "ppm", 2000.0,
    ...
    @Slot(str, float)
    def send_override(self, sensor_key: str, value: float):
        cmd = {"type": "override", "sensor": sensor_key, "value": value}
        self._write_cmd(cmd)
...
class LocalSubscribeTab(QWidget):
    ...
    def __init__(self, duplicator: InfluxDuplicator, parent=None):
        ...
        self.pbCO2 = self._mk_bar(0, 2000)
